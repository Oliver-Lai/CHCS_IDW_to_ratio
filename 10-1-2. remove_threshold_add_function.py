...
def plot_disease_by_clusters_slope_midpoint(disease_name, data_path, output_path):
    """
    為單一疾病生成 5 個子圖（每個群集一個）- 僅顯示三次擬合曲線與方程式
    """
    # 讀取疾病數據
    ...
    fig.suptitle(f'{disease_name} - Cubic Fit Analysis', 
    ...
    for idx, cluster_name in enumerate(CLUSTER_ORDER):
        ...
        # === 生成並顯示 Cubic 方程式 ===
        # coeffs 順序為 [a, b, c, d] 對應 ax^3 + bx^2 + cx + d
        a, b, c, d = result['coeffs']
        
        # 格式化方程式字串 (取四位小數以確保精準度)
        eq_text = (f"$f(x) = {a:.4e}x^3$\n"
                   f"$+ {b:.4e}x^2$\n"
                   f"$+ {c:.4e}x$\n"
                   f"$+ {d:.4f}$")
    ...
    # 保存圖片
    output_filename = f'Cubic_Fit_Only_{disease_name.replace(" ", "_")}_by_Cluster.png'
...
def main():
    ...
    output_path = "./7-1-2_Threshold_Analysis_Cubic"
